...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        const response = await axiosInstance.get('/admin/mentors', config);
  ...
  const handleVerification = async (mentorId, isVerified) => {
    try {
      await axiosInstance.put(`/admin/mentors/${mentorId}/verify`, { isVerified }, config);
    ...
    <div className="container mt-5">
      <div className="card shadow">
        <div className="card-header">
          <h2 className="text-center">Admin Panel - Restaurants</h2>
        </div>
